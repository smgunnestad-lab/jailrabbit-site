...
function getPhaseDetails(event, phase) {
  ...
  const phases = {
    registration: {
      ...
      message: "Drop your main account in the JailRabbit Discord before the doors close.",
      target: dates?.signupClose,
    },
    upcoming: {
      status: "Starting soon",
      label: "The race begins in",
      message: "Sign-ups are closed. Get the snacks ready while we prepare the starting line.",
      target: dates?.start,
    },
    live: {
      status: "Live now",
      label: "The race ends in",
      message: "The climb is on. Cheer for your people and follow the standings below.",
      target: dates?.end,
    },
    ended: {
      status: "Final results",
      label: "Results stay up for",
      message: "GGs all around. The final standings remain available for one week.",
...
function createEventCard(event, now) {
  ...
  card.type = "button";
  card.className = `event-card event-card--${event.cardTheme || "default"}`;
  ...
  footer.textContent = dates
    ? `${formatEventDate(event, dates.start, false)} · Everyone welcome`
  ...
  card.append(artwork, status, eyebrow, title, description, footer, openLabel);
  card.addEventListener("click", () => showEvent(event));
  return card;
}

function renderEventCards(now = new Date()) {
  const visibleEvents = communityEvents.filter((event) => {
    const isArchived = getPhase(event, now) === "archived";
    return !(isArchived && event.hideAfterArchive);
  });

  eventsGrid.replaceChildren(...visibleEvents.map((event) => createEventCard(event, now)));
...
function renderActiveEvents(now = new Date()) {
  const activeEvents = communityEvents.filter((event) =>
    ["registration", "upcoming", "live"].includes(getPhase(event, now)),
  );

  activeEventsBar.hidden = activeEvents.length === 0;
  activeEventsTrack.classList.toggle("is-scrolling", activeEvents.length > 3);
  activeEventsTrack.replaceChildren();

  const tickerEvents = activeEvents.length > 3 ? [...activeEvents, ...activeEvents] : activeEvents;
  tickerEvents.forEach((event, index) => {
    const item = createTickerItem(event);
    if (index >= activeEvents.length) item.setAttribute("aria-hidden", "true");
...
function renderRules() {
  ...
  rankedRaceEvent.rules.forEach((rule, index) => {
...
function updateOpenEventState(now = new Date()) {
  ...
  const phase = getPhase(openEvent, now);
  const details = getPhaseDetails(openEvent, phase);
...
function showEvent(event) {
  ...
  document.getElementById("modal-event-icon").textContent = event.icon || "🐇";
  document.getElementById("modal-event-eyebrow").textContent = event.eyebrow;
  document.getElementById("modal-event-title").textContent = event.title;
  document.getElementById("modal-event-intro").textContent = event.intro;

  const cozyNote = document.getElementById("modal-cozy-note");
  cozyNote.textContent = event.cozyNote || "";
  cozyNote.hidden = !event.cozyNote;

  rankedRaceContent.hidden = event.id !== rankedRaceEvent.id;
  ideaContent.hidden = event.id === rankedRaceEvent.id;
  eventDatesList.hidden = !dates;
  countdownCard.hidden = !dates;
  timezoneLabel.hidden = !dates;

  if (dates) {
    document.getElementById("signup-close-date").textContent = formatEventDate(event, dates.signupClose);
    document.getElementById("event-start-date").textContent = formatEventDate(event, dates.start, false);
    document.getElementById("event-end-date").textContent = formatEventDate(event, dates.end);
  }
...
function refreshTimeSensitiveUI() {
  const now = new Date();
  const phaseSignature = communityEvents.map((event) => getPhase(event, now)).join("|");
...
window.setInterval(refreshTimeSensitiveUI, 1000);

const linkedEvent = communityEvents.find((event) => `#${event.id}` === window.location.hash);
if (linkedEvent && getPhase(linkedEvent) !== "archived") showEvent(linkedEvent);
